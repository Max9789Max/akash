	...
	for {
		...
		case ev := <-o.sub.Events():
			...
			case event.TxCreateLease:
				...
				// different group
				if o.order.Compare(ev.Tx().GroupID()) != 0 {
					break
				}

				// check winning provider
				if bytes.Compare(o.session.Provider().Address, ev.Tx().Provider) != 0 {
					o.log.Info("lease lost", "lease-id", keys.LeaseID(ev.Tx().LeaseID).Path())
					break loop
				}

				// TODO: sanity check (price, state, etc...)

				o.log.Info("lease won", "lease-id", keys.LeaseID(ev.Tx().LeaseID).Path())

				o.bus.Publish(event.LeaseWon{
					LeaseID: ev.Tx().LeaseID,
					Group:   group,
					Price:   price,
				})
			...
			case event.TxCloseDeployment:
				...
				if bytes.Compare(o.order.Deployment, ev.Tx().Deployment) != 0 {
					break
				}
